    ...
    if n_size >= 3 and t_size >= 1:
        ...
        for t1 in range(0, int(math.floor(float(n_size + 4 * t_size - 5) / float(2))) + 1):
            ...
            for t2 in range(lbp, ubp + 1):
                if 2 * t1 == 4 * t2 - n_size + 2:
                    if (2 * t1 + 3 * n_size + 2) % 4 == 0:
                        b[n_size - 2] = S1(coef, a[n_size - 2 - 1], a[n_size - 2], a[n_size - 2 + 1])
                        a[n_size - 3] = S2(coef, b[n_size - 3 - 1], b[n_size - 3], b[n_size - 3 + 1])
                        a[n_size - 2] = S2(coef, b[n_size - 2 - 1], b[n_size - 2], b[n_size - 2 + 1])
                if t1 >= int(math.ceil(float(4 * t2 - n_size + 3) / float(2))) and t1 <= 2 * t2 - 1:
                    lbp = 2 * t2
                    ubp = 2 * t2 + 1
                    for t4 in range(2 * t2, 2 * t2 + 1 + 1):
                        b[-2 * t1 + 2 * t2 + t4] = S1(coef, a[-2 * t1 + 2 * t2 + t4 - 1], a[-2 * t1 + 2 * t2 + t4],
                            a[-2 * t1 + 2 * t2 + t4 + 1])
                        a[-2 * t1 + 2 * t2 + t4 - 1] = S2(coef, b[-2 * t1 + 2 * t2 + t4 - 1 - 1], b[-2 * t1 + 2 * t2 +
                            t4 - 1], b[-2 * t1 + 2 * t2 + t4 - 1 + 1])
                if t1 == 2 * t2:
                    if t1 % 2 == 0:
