...
@task(t2=IN, m_size=IN, t1=IN, n_size=IN, coef1=IN, coef2=IN, mat={Type: COLLECTION_INOUT, Depth: 2})
def LT2(t2, m_size, t1, n_size, coef1, coef2, mat):
    ...
    if m_size >= 1 and n_size >= 1:
        ...
        for t1 in range(lbp, ubp + 1):
            ...
            for t2 in range(0, int(math.floor(float(m_size - 1) / float(32))) + 1):
                ...
                LT2_aux_0 = [[mat[gv0][gv1] for gv1 in range(32 * t2, 3 * 2 + 32 * t2 if n_size >= 3 + 2 * t1 and 
                    m_size >= 3 * 3 + 32 * t2 else m_size if n_size >= 3 + 2 * t1 and 32 * t2 <= m_size <= 3 * 2 + 
                    32 * t2 else 3 * 2 + 32 * t2 if 2 * t1 <= n_size <= 2 + 2 * t1 and m_size >= 3 * 3 + 32 * t2 else
                    m_size, 1)] for gv0 in range(2 * t1, 2 + 2 * t1 if n_size >= 3 + 2 * t1 and m_size >= 3 * 3 + 32 *
                    t2 else 2 + 2 * t1 if n_size >= 3 + 2 * t1 and 32 * t2 <= m_size <= 3 * 2 + 32 * t2 else n_size if
                    2 * t1 <= n_size <= 2 + 2 * t1 and m_size >= 3 * 3 + 32 * t2 else n_size, 1)]
                LT2(t2, m_size, t1, n_size, coef1, coef2, LT2_aux_0)
    compss_barrier()
    if __debug__:
        mat = compss_wait_on(mat)
        print('New Matrix:')
