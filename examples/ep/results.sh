#!/bin/bash -e
  ...
  echo -e "JOB_ID\\tVERSION\\t\\tNSIZE\\tMSIZE\\tTRACING\\tNUM_WORKERS\\tTOTAL_TIME\\tINIT_TIME\\tCOMP_TIME\\tNUM_TASKS" > "${job_results_file}"
  ...
  while read -r line; do
    ...
    comp_time=$(grep "EP_TIME" "${job_output}" | awk '{ print $NF }' | cat)
    ...
    if [ "${move_traces}" == "true" ] && [ "$tracing" == "true" ]; then
      ...
      new_trace_basename=fdtd1d-${version}-${job_id}-${nsize}-${msize}
