#!/bin/bash -e
  ...
  echo "JOB_ID	VERSION		NSIZE	MSIZE	TRACING" > "${job_log_file}"
  ...
  for i in "${!NSIZES[@]}"; do
    ...
    for app_path in "${SCRIPT_DIR}"/*/; do
      ...
      if [ "${app_version}" != "results" ]; then
        ...
        # With tracing
        tracing=true
        ./enqueue.sh "${app_version}" "${job_dependency}" "${num_nodes}" "${execution_time}" "${cpus_per_node}" "${tracing}" "${graph}" "${log_level}" "${nsize}" "${msize}"
        wait_and_get_jobID
        log_information "${job_log_file}" "${job_dependency}" "${app_version}" "${nsize}" "${msize}" "${tracing}"
        # Without tracing
        tracing=false
        ./enqueue.sh "${app_version}" "${job_dependency}" "${num_nodes}" "${execution_time}" "${cpus_per_node}" "${tracing}" "${graph}" "${log_level}" "${nsize}" "${msize}"
        wait_and_get_jobID
