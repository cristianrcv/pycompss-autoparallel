...
def matmul(a, b, c, m_size, alpha, beta):
    ...
    if m_size >= 1:
        ...
        for t2 in range(lbp, ubp + 1):
            lbp = 0
            ubp = int(math.floor(float(m_size - 1) / float(2)))
            LT3_aux_0 = [[c[gv0][gv1] for gv1 in range(0, m_size if m_size >= 0 and -1 + m_size <= 2 * t2 <= m_size else
                m_size, 1)] for gv0 in range(2 * t2, m_size if m_size >= 0 and -1 + m_size <= 2 * t2 <= m_size else 
                2 + 2 * t2, 1)]
            ...
            for gv0 in range(2 * t2, m_size if m_size >= 0 and -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t2, 1):
                for gv1 in range(0, m_size if m_size >= 0 and -1 + m_size <= 2 * t2 <= m_size else m_size, 1):
        ...
        for t2 in range(lbp, ubp + 1):
            ...
            for t3 in range(0, int(math.floor(float(m_size - 1) / float(2))) + 1):
                ...
                for t4 in range(0, int(math.floor(float(m_size - 1) / float(2))) + 1):
                    ...
                    for gv0 in range(2 * t2, m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <= -2 + m_size and 
                        -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t2 if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <=
                        -2 + m_size and 2 * t2 <= -2 + m_size else m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and -1 +
                        m_size <= 2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t2 if 2 * t3 <=
                        m_size <= 2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 2 * t2 <= -2 + m_size else 
                        m_size if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and -1 + m_size <= 2 * t2 <= m_size
                         else 2 + 2 * t2 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and 2 * t2 <= -2 + m_size
                         else m_size if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and -1 + m_size <= 
                        2 * t2 <= m_size else 2 + 2 * t2, 1):
                        for gv1 in range(2 * t4, 2 + 2 * t4 if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <= -2 +
                            m_size and -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t4 if 2 * t3 <= m_size <= 2 + 2 *
                            t3 and 2 * t4 <= -2 + m_size and 2 * t2 <= -2 + m_size else m_size if 2 * t3 <= m_size <=
                            2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else 
                            m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 2 * t2 <=
                            -2 + m_size else 2 + 2 * t4 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and -1 +
                            m_size <= 2 * t2 <= m_size else 2 + 2 * t4 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 +
                            m_size and 2 * t2 <= -2 + m_size else m_size if m_size >= 3 + 2 * t3 and -1 + m_size <= 
                            2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else m_size, 1):
                            a[gv0][gv1] = LT4_aux_0[gv0 - 2 * t2][gv1 - 2 * t4]
                    for gv0 in range(2 * t4, 2 + 2 * t4 if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <= -2 + m_size and
                        -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t4 if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <=
                        -2 + m_size and 2 * t2 <= -2 + m_size else m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and -1 +
                        m_size <= 2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else m_size if 2 * t3 <=
                        m_size <= 2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 2 * t2 <= -2 + m_size else 2 + 
                        2 * t4 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and -1 + m_size <= 2 * t2 <= m_size
                         else 2 + 2 * t4 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and 2 * t2 <= -2 + m_size
                         else m_size if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and -1 + m_size <= 
                        2 * t2 <= m_size else m_size, 1):
                        for gv1 in range(2 * t3, m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <= -2 + m_size and
                            -1 + m_size <= 2 * t2 <= m_size else m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <=
                            -2 + m_size and 2 * t2 <= -2 + m_size else m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and 
                            -1 + m_size <= 2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else m_size if 2 *
                            t3 <= m_size <= 2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 2 * t2 <= -2 + m_size
                             else 2 + 2 * t3 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and -1 + m_size <= 2 *
                            t2 <= m_size else 2 + 2 * t3 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and 2 *
                            t2 <= -2 + m_size else 2 + 2 * t3 if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <=
                            m_size and -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t3, 1):
                            b[gv0][gv1] = LT4_aux_1[gv0 - 2 * t4][gv1 - 2 * t3]
                    for gv0 in range(2 * t2, m_size if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 
                        -1 + m_size <= 2 * t2 <= m_size else m_size if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 +
                        m_size and -1 + m_size <= 2 * t2 <= m_size else m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and
                        -1 + m_size <= 2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else m_size if 2 * t3 <=
                        m_size <= 2 + 2 * t3 and 2 * t4 <= -2 + m_size and -1 + m_size <= 2 * t2 <= m_size else 2 + 
                        2 * t2 if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 2 * t2 <= -2 + m_size
                         else 2 + 2 * t2 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and 2 * t2 <= -2 + m_size
                         else 2 + 2 * t2 if 2 * t3 <= m_size <= 2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and 2 *
                        t2 <= -2 + m_size else 2 + 2 * t2, 1):
                        for gv1 in range(2 * t3, 2 + 2 * t3 if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <=
                            m_size and -1 + m_size <= 2 * t2 <= m_size else 2 + 2 * t3 if m_size >= 3 + 2 * t3 and 2 *
                            t4 <= -2 + m_size and -1 + m_size <= 2 * t2 <= m_size else m_size if 2 * t3 <= m_size <=
                            2 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and -1 + m_size <= 2 * t2 <= m_size else 
                            m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and 2 * t4 <= -2 + m_size and -1 + m_size <= 2 *
                            t2 <= m_size else 2 + 2 * t3 if m_size >= 3 + 2 * t3 and -1 + m_size <= 2 * t4 <= m_size and
                            2 * t2 <= -2 + m_size else 2 + 2 * t3 if m_size >= 3 + 2 * t3 and 2 * t4 <= -2 + m_size and
                            2 * t2 <= -2 + m_size else m_size if 2 * t3 <= m_size <= 2 + 2 * t3 and -1 + m_size <= 2 *
                            t4 <= m_size and 2 * t2 <= -2 + m_size else m_size, 1):
