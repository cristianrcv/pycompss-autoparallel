#!/bin/bash -e
  ...
  log_information() {
    ...
    echo "$job_id	$version	$m_size $tracing" >> "${job_log_file}"
  }
  ...
  echo "JOB_ID	VERSION		MSIZE	TRACING" > "${job_log_file}"
  ...
  for i in "${!MSIZES[@]}"; do
    ...
    for app_path in "${SCRIPT_DIR}"/*/; do
      ...
      # With tracing
      tracing=true
      ./enqueue.sh "${app_version}" "${job_dependency}" "${num_nodes}" "${execution_time}" "${cpus_per_node}" "${tracing}" "${graph}" "${log_level}" "${msize}"
      wait_and_get_jobID
      log_information "${job_log_file}" "${job_dependency}" "${app_version}" "${msize}" "${tracing}"
      # Without tracing
      tracing=false
      ./enqueue.sh "${app_version}" "${job_dependency}" "${num_nodes}" "${execution_time}" "${cpus_per_node}" "${tracing}" "${graph}" "${log_level}" "${msize}"
      wait_and_get_jobID
