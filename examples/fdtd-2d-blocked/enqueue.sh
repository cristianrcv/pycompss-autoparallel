#!/bin/bash -e                                                                                                                                                                                                                                                                 
  ...
  EXEC_FILE=${SCRIPT_DIR}/${app_version}/fdtd-2d-blocked.py
  ...
  enqueue_compss \
    ...
    --lang=python \
    \
    "$EXEC_FILE" "$nsize" "$bsize" "$tsize"

  # Params: job_dependency version num_nodes execution_time cpus_per_node tracing graph log_level NSIZE BSIZE TSIZE
