#!/bin/bash -e
  ...
  echo -e "JOB_ID\\tVERSION\\t\\tMSIZE\\tBSIZE\\tTSIZE\\tTRACING\\tNUM_WORKERS\\tTOTAL_TIME\\tINIT_TIME\\tCOMP_TIME\\tNUM_TASKS" > "${job_results_file}"
  ...
  while read -r line; do
    ...
    # Print results
    echo -e "${job_id}\\t${version}\\t${nsize}\\t${bsize}\\t${tsize}\\t${tracing}\\t${num_workers}\\t\\t${total_time}\\t${init_time}\\t${comp_time}\\t${num_tasks}" >> "${job_results_file}"
    ...
    if [ "${move_traces}" == "true" ] && [ "$tracing" == "true" ]; then
      ...
      new_trace_basename=fdtd2dblocked-${version}-${job_id}-${nsize}-${bsize}-${tsize}
