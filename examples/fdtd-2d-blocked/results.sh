#!/bin/bash -e
  ...
  echo "JOB_ID		VERSION		NSIZE  BSIZE	TSIZE	TRACING	TOTAL_TIME	INIT_TIME	COMP_TIME	NUM_TASKS" > "${job_results_file}"
  ...
  while read -r line; do
    ...
    # Print results
    echo "${job_id}       ${version}        ${nsize}	${bsize}    ${tsize}	${tracing}	${total_time}	${init_time}	${mult_time}	${num_tasks}" >> "${job_results_file}"
