#!/bin/bash -e
  ...
  echo "JOB_ID		VERSION		NSIZE	TRACING	TOTAL_TIME	INIT_TIME	COMP_TIME	NUM_TASKS" > "${job_results_file}"
  ...
  while read -r line; do
    ...
    mult_time=$(grep "FLOYD_TIME" "${job_output}" | awk '{ print $NF }' | cat)
    num_tasks=$(grep "Total executed tasks:" "${job_output}" | awk '{ print $NF }' | cat)

    # Print results
    echo "${job_id}       ${version}        ${nsize}	${tracing}	${total_time}	${init_time}	${mult_time}	${num_tasks}" >> "${job_results_file}"
    ...
    if [ "${move_traces}" == "true" ] && [ "$tracing" == "true" ]; then
      ...
      new_trace_basename=floyd-${version}-${job_id}-${nsize}
