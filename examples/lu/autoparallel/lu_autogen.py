...
def lu_blocked(a, m_size, b_size):
    ...
    if m_size >= 2:
        ...
        for t1 in range(1, m_size - 2 + 1):
            ...
            lbp = 0
            ubp = t1 - 1
            for t2 in range(0, t1 - 1 + 1):
                aux[0] = S9(u_mat[t2][t2])
                l_mat[t1][t2] = S10(p_mat[t1][t1], a[t1][t2], aux[0])
            lbp = t1
            ubp = m_size - 1
            for t3 in range(t1, m_size - 1 + 1):
                aux[0] = S5(u_mat[t1 - 1][t1 - 1])
                aux2[0] = S6(a[t1][t1 - 1], aux[0])
                a[t1][t3] = S7(a[t1][t3], aux2[0], u_mat[t1 - 1][t3])
            lbp = t1 + 1
            ubp = m_size - 1
            for t2 in range(t1 + 1, m_size - 1 + 1):
                aux[0] = S11(l_mat[t1][t1])
                u_mat[t1][t2] = S12(aux[0], p_mat[t1][t1], a[t1][t2])
                lbp = t1
                ubp = m_size - 1
                for t3 in range(t1, m_size - 1 + 1):
                    aux[0] = S5(u_mat[t1 - 1][t1 - 1])
    ...
    compss_barrier()
    if __debug__:
        p_res = compss_wait_on(p_mat)
        l_res = compss_wait_on(l_mat)
        u_res = compss_wait_on(u_mat)
        print('Matrix P:')
        print(p_res)
        print('Matrix L:')
        print(l_res)
