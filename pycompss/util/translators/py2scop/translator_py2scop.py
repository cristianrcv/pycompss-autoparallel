...
class Py2Scop(object):
        ...
        def translate(self, baseFileName):
                ...
                for scop in self.scops:
                        ...
                        try:
                                ...
                                if __debug__:
                                        logger.debug("[py2scop] Scop written to " + str(fileName))
                        except Exception as e:
        ...
        def _ast2scop(tree):
                ...
                iter_vars_list = list(iter_vars_set)
                # Array variables
        ...
        def _get_statements(node, for_fathers, param_vars, all_vars):
                ...
                for _, value in ast.iter_fields(node):
                        if isinstance(value, list):
                                for item in value:
                                        if isinstance(item, ast.AST):
                                                statements_scop.extend(Py2Scop._get_statements(item, for_fathers, param_vars, all_vars))
                        elif isinstance(value, ast.AST):
                                statements_scop.extend(Py2Scop._get_statements(value, for_fathers, param_vars, all_vars))
        ...
        def _process_statement(statement_loop, fathers_loop, param_vars, all_vars):
                ...
                domain_scop = Relation(RelationType.DOMAIN, domain_rows, domain_cols, domain_out_dims, domain_in_dims, domain_local_dims, domain_num_pars, domain_matrix)
        ...
        def _process_accesses(node, iter_vars, param_vars, all_vars):
                ...
                if isinstance(node, _ast.Name):
                        ...
                        from pycompss.util.translators.scop_types.scop.statement.relation_class import RelationType
                        accesses.append(Py2Scop._process_access(node, RelationType.READ, iter_vars, param_vars, all_vars))
        ...
        def _process_access(node, access_type, iter_vars, param_vars, all_vars):
                ...
                from pycompss.util.translators.scop_types.scop.statement.relation_class import Relation
                access = Relation(access_type, access_rows, access_cols, access_out_dims, access_in_dims, access_local_dims, access_num_pars, access_matrix)
                return access
        ...
        def _process_expr(node, names2index, dim):
                ...
                if isinstance(node, _ast.Name):
                        ...
                        if isinstance(node.op, _ast.Sub):
                                ...
                                leftIsCnst = (res_left[:-1] == [0]*(dim - 1))
                                rightIsCnst = (res_right[:-1] == [0]*(dim - 1))
                                if not leftIsCnst and not rightIsCnst:
